...
<div id="article">
  ...
  {foreach $single as $s}
  <form name="formview" id="formview" action="{$base_url}admin/{$model_type}/edit/{$content.id}" method="post"   enctype="multipart/form-data">
    {/foreach}
    {foreach $single as $s}
    <input type="hidden" name="model" value="{$s.id}" />
    {/foreach}
    <table class="content_list" width="100%">
      <tbody>{if isset($has_category)  && !empty($category)}
        <tr>
      ...
      <tr>
        ...
        {if $m.type eq 1}
        <td colspan="4"><input name="{$m.tablename}" id="title"  size="30" class="input-text number" validtip="required" value="{$content[$m.tablename]}" type="text"></td>
        {elseif $m.type eq 2}
        <td colspan="4"><input name="{$m.tablename}" id="title"  size="60" style="color:" class="validate input-text" validtip="required" value="{$content[$m.tablename]}" type="text"></td>
        {elseif $m.type eq 3}
         <td colspan="4"><textarea id="text" name="{$m.tablename}" style="width:600px;height:130px;">{$content[$m.tablename]}</textarea></td>
        {elseif $m.type eq 4}
        <td colspan="4"><textarea class="editor" name="{$m.tablename}" style="width:700px;height:300px;" >{$content[$m.tablename]}</textarea></td>
        {elseif $m.type eq 5}
        <td colspan="4"><input class="Wdate" type="text" name="{$m.tablename}" value="{$content[$m.tablename]}" onClick="WdatePicker({ skin:'whyGreen',doubleCalendar:true,dateFmt:'yyyy-MM-dd HH:mm:ss' })"></td>
        {elseif $m.type eq 6}
