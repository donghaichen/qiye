...
<div id="article">
  <div class="tab_menu">
    <ul>
      <li class="current"><a href="{$base_url}admin/modelmanager">内容模型管理 </a></li>
    </ul>
  </div>
  <div class="tab_content_wrap">
    <div class="tab_content">
      <form id="nav_set" method="post" action="{$base_url}admin/modelmanager/add" name="nav_set">
        <table id="table_nav_list" class="content_list" width="100%"  cellspacing="0" >
          ...
          <tr>
            ...
            <td><a href="{$base_url}admin/modelmanager/del/{$data.id}" class="act">删除</a></td>
          </tr>
          {/if} 
          ...
          <tr>
            ...
            <td class="order"><div class="board">
                <input class="txt" size="2" name="sortid[]" value="{$sub.sortid}" type="text">
              </div></td>
            ...
            <td>{foreach $model_type as $type_data}{if $sub.type == $type_data.id}{$type_data.name}{/if}{/foreach}{if isset($sub.datasource)} -> {$sub.datasource}{/if}</td>
            ...
            <td><a href="{$base_url}admin/modelmanager/del/{$sub.id}" class="act">删除</a></td>
          </tr>
          {/if}
          {/foreach}
      ...
      <script type="text/javascript">
		...
		function addchild(obj,parentid,field)
		{
			...
			var td5 = $('<td></td>');
			td5.append($('<select name="type[]" class="scombo">{foreach $model_type as $type_data}<option {if $type_data.id == 0} selected{/if}  value="{$type_data.id}">{$type_data.name}</option>{/foreach}</select><input name="size[]" value="255" class="field-txt" type="text"/> '));	
